...
func pixelateImage(image: CGImage) throws -> CGImage {
    guard let device = MTLCreateSystemDefaultDevice() else {
        throw RuntimeError(message: "Failed to create GPU device.")
    }
    let textureLoader = MTKTextureLoader(device: device)
    let texture = try textureLoader.newTexture(cgImage: image, options: [MTKTextureLoader.Option.SRGB : false])
    
    guard let commandQueue = device.makeCommandQueue() else {
        throw RuntimeError(message: "Failed to setup metal pipeline.")
    }
    
    let metallibURL = URL(fileURLWithPath: "/Users/aybars/Developer/PixelateCLI/.build/debug/default.metallib")
    ...
    let im = context.createCGImage(ciImage, from: CGRect(x: 0, y: 0, width: texture.width, height: texture.height))!
